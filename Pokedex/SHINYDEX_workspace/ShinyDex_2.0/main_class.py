...
def add_2_shiny_db():
    # Define the properties for the ShinyPokemon class
    class ShinyPokemon:
        def __init__(self, entry, dex_no, pokemon, variation, form, gender, og_game, location, cap_date, method, nickname, nature, pokeball, encounts, descript, trainer):
            self.entry_no = entry 
            self.dex_number = dex_no
            self.pokemon = pokemon
            self.variation = variation.strip().title() # title?
            self.form = form
            self.gender = gender 
            self.og_game = og_game
            self.location = location
            self.cap_date = pd.to_datetime(cap_date)
            self.method = hunt_entry.get() 
            self.nickname = nickname.strip() if nickname else self.name
            self.nature = natr_menu.get()
            self.pokeball = pokeball.strip().title()
            self.encounters = encounts
            self.description = descript.strip()
        ...
        def to_dict(self):
            ...
            return {
                ...
                "DexNo": self.dex_number,
                "Pokemon": self.pokemon,
                "Variation": self.variation,
                "Form": self.form,
                "Gender": self.gender,
                "OriginalGame": self.og_game,
                "Location": self.location,
                "DateOfCapture": self.cap_date,
                "Method": self.method,
    ...
    # Gather user input
    ## import dex to add new entry and drop bad columns
    ...
    # get selected Pokemon from mon_listbox index, as tuple
    selected_indices = mon_listbox.curselection() 
    # check if the user has selected a mon
    if selected_indices:
        index = selected_indices[0] 
        mon = mon_listbox.get(index)
        
        # if the user input a mon variation, ascertain that value makes sense. 
        # clear variat variable if variation does not exist in the national dex
        if variat_menu.get() == "Alolan":
            var_mon = nat_dex.loc[(nat_dex["Pokemon"] == mon) & (nat_dex["Region"] == 'Alola')]
            if len(var_mon) == 0:
                variat = '' 
            else:
                variat = variat_menu.get()
        elif variat_menu.get() == "Galarian":
            var_mon = nat_dex.loc[(nat_dex["Pokemon"] == mon) & (nat_dex["Region"] == 'Galar')]
            if len(var_mon) == 0:
                variat = '' 
            else:
                variat = variat_menu.get()
        elif variat_menu.get() == "Hisuian":
            var_mon = nat_dex.loc[(nat_dex["Pokemon"] == mon) & (nat_dex["Region"] == 'Hisui')]
            if len(var_mon) == 0:
                variat = '' 
            else:
                variat = variat_menu.get()
        elif variat_menu.get() == "Paldean":
            var_mon = nat_dex.loc[(nat_dex["Pokemon"] == mon) & (nat_dex["Region"] == 'Paldea')]
            if len(var_mon) == 0:
                variat = '' 
            else:
                variat = variat_menu.get()
        ...
        dex_no = mon_deets.iloc[0]['NdexNo']
        
    else: # later, see if we can display error on gui and retry the entry without restarting the whole ui
        mon = 'Fake Mon'
        dex_no = 0 
    ...
    # deal with dates out of range later
    
    # Create the new entry from user input
    new_catch = ShinyPokemon(
        ...
        cap_date = date_entry.get(), 
        method = hunt_entry.get(),
        nickname = ni_name_entry.get(),
        nature = natr_menu.get(),
        pokeball = ball_menu.get(), 
        encounts = encount_entry.get(), # if it's a string.., do something
        descript= description.get("1.0", "end-1c"),
        trainer = "Fake-o Joe"
    )
    
    # Save to Excel using Pandas
    new_entry = pd.DataFrame([new_catch.to_dict()])
    
    try:
        # Append to existing excel file if it exists
        final_df = pd.concat([dex_raw, new_entry], ignore_index = True)
    except FileNotFoundError:
        final_df = new_entry
    
    final_df.to_excel("Shiny_Dex_Out.xlsx", index = False)
...
mon_list = nat_dex["Pokemon"].sort_values().unique()
